...
CREATE TABLE VaiTro (
    id_vaitro INT AUTO_INCREMENT PRIMARY KEY, -- Mã số định danh duy nhất cho mỗi vai trò.
...
CREATE TABLE TaiKhoan (
    id_taikhoan INT AUTO_INCREMENT PRIMARY KEY, -- Khóa chính: Mã duy nhất cho mỗi tài khoản.
...
CREATE TABLE BienDongNhanKhau (
    id_biendong INT AUTO_INCREMENT PRIMARY KEY, -- Mã số định danh duy nhất cho mỗi lần biến động.
...
CREATE TABLE KhoanThu (
    id_khoanthu INT AUTO_INCREMENT PRIMARY KEY, -- Mã số định danh duy nhất cho mỗi loại phí.
...
CREATE TABLE DotThuPhi (
    id_dotthu INT AUTO_INCREMENT PRIMARY KEY,   -- Mã số định danh duy nhất cho mỗi đợt thu.
    ...
    trang_thai VARCHAR(50),                   -- Trạng thái (khớp với sơ đồ).
    id_khoanthu INT,
    
    FOREIGN KEY (id_khoanthu) REFERENCES KhoanThu(id_khoanthu)
);
...
CREATE TABLE HoaDon (
    id_hoadon INT AUTO_INCREMENT PRIMARY KEY,     -- Mã số định danh duy nhất cho mỗi hóa đơn.
    ...
    id_chuho INT,                             -- Khóa ngoại, xác định hóa đơn này của hộ nào (đổi tên từ 'id_hokhau').
    id_dotthu INT,                            -- Khóa ngoại, cho biết hóa đơn thuộc đợt thu nào.
    
    FOREIGN KEY (id_chuho) REFERENCES HoKhau(id_hokhau),
